...
if __name__ == "__main__":
    ...
    # evaluation_path ='css_word_embedding/stereotype/antonmy/SD_evaluation_pairs.csv'
    # potency_path = 'css_word_embedding/stereotype/antonmy/SD_potency_pairs.csv'
    
    # aff_path = '/Users/xuanlong/Documents/program/python/src/xuanlong/alg/css_word_embedding/GeometryofCulture/code/word_pairs/affluence_pairs.csv'
    
    ant_pairs_warmth = get_antonym_pairs(warmth_path)
    ant_pairs_competent = get_antonym_pairs(competent_path)
    # ant_pairs_evaluation = get_antonym_pairs(evaluation_path)
    # ant_pairs_potency = get_antonym_pairs(potency_path)
    
    warm_pair = filter_pairs(ant_pairs_warmth)
    competent_pair = filter_pairs(ant_pairs_competent)
    # evaluation_pair = filter_pairs(ant_pairs_evaluation)
    # potency_pair = filter_pairs(ant_pairs_potency)
    
    dir_warmth = build_dimensions(warm_pair)
    dir_competent = build_dimensions(competent_pair)
    # dir_evaluation = build_dimensions(evaluation_pair)
    # dir_potency = build_dimensions(potency_pair)
    
    print(dir_warmth,dir_competent)

    cos_sim = cosine_similarity(dir_warmth,dir_competent)
    # cos_sim = cosine_similarity(dir_evaluation,dir_potency)
    print(cos_sim)
    
    dims = np.vstack((dir_warmth,dir_competent))
    # dims = np.vstack((dir_evaluation,dir_potency))
